...
export function loginFacebook() {
  return (dispatch) => {
    ...
    manager
      .authorize('facebook', { scopes: 'public_profile' })
      .then((resp) => {
        const data = {
          provider: 'facebook',
        ...
        DevSummitAxios.post('/auth/login', data, { headers })
          .then(async (response) => {
            if (response && response.data && response.data.meta.success) {
              ...
              Alert.alert('Login Failed', response.data.meta.message);
              axios
                .get('https://graph.facebook.com/me?fields=id,first_name,last_name,email', {
                  headers: {
                    Accept: 'application/json',
      ...
      .catch((err) => {
        console.log('error login fb', err);
      });
  };
}
